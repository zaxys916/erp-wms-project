...
const current = ref<SalesOrderDetail | null>(null)
const formRef = ref<FormInstance>()

const form = reactive({ customer_id: null as number | null, remark: '' })
...
async function loadBase() {
  const [cus, prod, zone] = await Promise.all([customerAPI.list(), productAPI.list(), zoneAPI.list()])
  customers.value = cus.data.items
...
function openCreate() {
  Object.assign(form, { customer_id: null, remark: '' })
...
async function submit() {
  ...
  try {
    await saleAPI.create({ customer_id: form.customer_id!, remark: form.remark, items: payload })
...
async function approve(row: SalesOrder) {
  try {
    await saleAPI.approve(row.id)
    ElMessage.success('销售订单已审核')
    await loadData()
  } catch (e) {
    ElMessage.error(extractError(e))
  }
...
async function ship(row: SalesOrder) {
  try {
    await ElMessageBox.confirm(`确认对销售单「${row.order_no}」发货出库？`, '提示', { type: 'warning' })
  } catch {
    return
  }
  try {
    await saleAPI.ship(row.id)
    ElMessage.success('已发货并扣减库存')
    await loadData()
  } catch (e) {
    ElMessage.error(extractError(e))
  }
...
async function cancel(row: SalesOrder) {
  try {
...
async function showDetail(row: SalesOrder) {
  try {
    current.value = await saleAPI.get(row.id).then((r) => r.data)
...
<template>
  <div class="page">
      ...
      <el-table :data="rows" v-loading="loading" border stripe>
        ...
        <el-table-column label="操作" width="240" fixed="right">
          <template #default="{ row }">
            ...
            <el-button v-if="row.status === 'approved'" size="small" type="success" @click="ship(row)">发货</el-button>
    ...
    <!-- 新建销售订单 -->
    <el-dialog v-model="dialogVisible" title="新建销售订单" width="720px">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="客户" prop="customer_id">
          <el-select v-model="form.customer_id" placeholder="选择客户" style="width: 100%">
    ...
    <!-- 详情 -->
    <el-dialog v-model="detailVisible" title="销售订单明细" width="640px">
