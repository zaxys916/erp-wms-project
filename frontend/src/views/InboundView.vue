...
async function approve(row: StockOrder) {
  try {
    await ElMessageBox.confirm(`确认审核通过入库单 ${row.order_no}？审核后库存将增加 ${row.quantity}。`, '审核入库单', {
      type: 'warning'
    })
...
async function reject(row: StockOrder) {
  try {
    await ElMessageBox.confirm(`确认驳回入库单 ${row.order_no}？`, '驳回入库单', { type: 'warning' })
...
async function cancelOrder(row: StockOrder) {
  try {
    await ElMessageBox.confirm(`确认取消入库单 ${row.order_no}？`, '取消入库单', { type: 'warning' })
...
<template>
  <div>
    <el-alert
      ...
      :closable="false"
      show-icon
      title="入库流程：创建入库单 → 审核通过后库存自动增加，并记录库存流水。"
      class="mb"
    />

    <el-card shadow="never" class="mb">
      <template #header><span>创建入库单</span></template>
      <el-form ref="formRef" :model="form" :rules="rules" label-width="90px" style="max-width: 560px">
        ...
        <el-form-item label="目标库位" prop="zone_id">
        ...
        <el-form-item>
          <el-button type="primary" :loading="submitting" @click="submit">提交入库单</el-button>
        </el-form-item>
      </el-form>
    </el-card>
